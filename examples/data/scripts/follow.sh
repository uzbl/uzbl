...
if ! which socat >/dev/null 2>&1; then
  echo 'script @scripts_dir/follow.js "@{follow_hint_keys} '$8'"' > "$UZBL_FIFO"
  exit
fi

result=$(echo 'script @scripts_dir/follow.js "@{follow_hint_keys} '$8'"' | socat - unix-connect:"$UZBL_SOCKET")
case $result in
